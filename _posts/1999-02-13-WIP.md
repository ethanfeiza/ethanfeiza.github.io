...
cc<sub>11</sub>: starting from state S<sub>1</sub>, we stay in state S<sub>1</sub>.
cc<sub>12</sub>: starting from state S<sub>1</sub>, we transition to state S<sub>2</sub>.
cc<sub>13</sub>: starting from state S<sub>1</sub>, we transition to state S<sub>3</sub>.

![]({{ site.url }}{{ site.baseurl }}/images/MarkovChain/TransitionSet.png)<!-- -->

Each transition c<sub>ij</sub> has a *transition probability*: the likelihood of transitioning from state *i* to state *j*. The probability transition c<sub>ij</sub> can be written as:

P(c<sub>ij</sub>) = P(s<sub>t+1</sub> = *j* | s<sub>t</sub> = *i*) = P<sub>ij</sub>
