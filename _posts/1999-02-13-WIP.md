...
Each transition c<sub>ij</sub> has a *transition probability*: the likelihood of transitioning from state i to state j. The transition probability c<sub>ij</sub> can be written as:<br>
<br>
**P(c<sub>ij</sub>) = P(s<sub>t+1</sub> = *s<sub>j</sub>* | s<sub>t</sub> = *s<sub>i</sub>*) = P<sub>ij</sub>**, where *t* indicates a step or transition in the system.
