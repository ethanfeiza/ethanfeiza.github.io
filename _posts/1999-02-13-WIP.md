...
A transition from state *i* to state *j* is written as *i*→*j*. Let's pick a random state in our system like S<sub>1</sub>. From S<sub>1</sub>, there exist three possible transitions:
...
<br>
**P(*i*→*j*) = P(s<sub>t+1</sub> = *s<sub>j</sub>* | s<sub>t</sub> = *s<sub>i</sub>*) = P<sub>ij</sub>** where *t* indicates a state-step in the system.<br>
To derive an individual transition probability, we can create an estimate $\hat{P}_{ij}$ using observed data in the system. Let's say we've been monitoring the system in our example for some time, and we've counted 10 total transitions leaving S<sub>1</sub>. Then 
