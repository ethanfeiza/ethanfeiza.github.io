...
Before we dive into a real-world application of a Markov Chain, let's familiarize ourselves with some basic concepts. For any stochastic process, we must define a *State Space*: a complete set of states covering all possible system outcomes. For example, lets define a simple system with 3 total states.
