...
<br>
**P<sub>ij</sub> = P(S<sub>*i*→*j*</sub>) = P(S<sub>t+1</sub> = S<sub>j</sub> | S<sub>t</sub> = S<sub>i</sub>) where t represents a step in the system.** where *t* indicates a state-step in the system.<br>

In order to estimate the probability of a transition occuring, P_HAT<sub>ij</sub>, we must first observe the system and count the number of transitions from *i*→*j* that have occurred. These observable counts are denoted c<sub>ij</sub>.

![]({{ site.url }}{{ site.baseurl }}/images/MarkovChain/TransitionCounts.png)<!-- -->

The second requirement is counting the number of transitions from *i*→*k* that have occurred, where *k* denotes any possible transition state from *i*. We can denote the total count of transitions from *i* as c<sub>ik</sub>. Now, we're ready to estimate the transition probability P_Hat<sub>ij</sub>.

\hat{P}_{ij}
P&#770;<sub>ij</sub>
<span>P̂</span><sub>ij</sub>
