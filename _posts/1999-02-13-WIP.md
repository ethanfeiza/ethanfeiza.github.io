...
A *Markov Chain* is a mathematical model that uses this property to predict the next state of a system given only its current state. Despite the simplicity in requiring no memory of past states, this approach can prove powerful across many applications, like natural language processing or predicting how users navigate websites.
...
<br>
**P<sub>ij</sub> = P(S<sub>i</sub>→S<sub>j</sub>) = P(S<sub>t+1</sub> = S<sub>j</sub> | S<sub>t</sub> = S<sub>i</sub>) where t represents a step in the system.** where *t* indicates a state-step in the system.<br>
...
Note that the sum of transition probabilities from any state sums to one. This confirms every possible state transition is accounted for from the current state.

## Application

In summary, a Markov chain is typically chosen to model systems defined by discrete states and probabilistic movement between them where future behavior depends only on the present state. This makes Markov chains effective for navigation, occupancy, and regime-switching dynamics.
