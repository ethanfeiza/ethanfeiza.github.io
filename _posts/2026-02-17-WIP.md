...
You might notice a small issue. Because no transitions exist originating *from* several states above, the denominator in this formula summing total counts *from* those states is zero. To avoid dividing by zero and corrupting our transition probability matrix, we can introduce a concept known as Laplace smoothing. This adds a small constant to every transition count, ensuring no probability is zero and all rows sum to one. This stabilizes estimates when some transitions are missing in the data.

$$
\hat{P}_{ij} = \frac{c_{ij} + \alpha}{\sum_k (c_{ik} + \alpha)}
$$

We will select an alpha value of 0.1 for smoothing. Now, we can build out our transition probability matrix.
