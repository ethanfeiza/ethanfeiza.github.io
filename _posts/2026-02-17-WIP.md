...
# Actual percentage of checkout sessions that visited CATEGORY
checkout_sessions = df[df["next_state"] == "checkout"]["session_id"].unique()
checkout_df = df[df["session_id"].isin(checkout_sessions)]
sessions_with_category = checkout_df[checkout_df["state"] == "category"]["session_id"].unique()
pct_through_category = len(sessions_with_category) / len(checkout_sessions)

# Compare estimate vs realized probability
print(f"Markov chain estimate: {p_category_given_checkout:.2%}")
print(f"Observed probability: {pct_through_category:.2%}")
```

Markov Chain Estimate: 59.83% <br>
Observed Probability: 44.07% <br>

The model suggests more sessions pass through category than the observed data shows. That gap exists because the Markov chain smooths behavior and allows all possible paths, while real users might take shortcuts or skip pages. This difference highlights the limits of the model and shows where real behavior diverges from its assumptions.
