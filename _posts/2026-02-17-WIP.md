...
# Drop transitions from absorbing to absorbing states (checkout->checkout or bounce->bounce)
absorbing_states = ["checkout", "bounce"]
df = df[~((df["state"].isin(absorbing_states)) & (df["next_state"].isin(absorbing_states)))]
...
<< notes above...>>
<< need & (df["next_state"].isin(absorbing_states) in the last step? >>
<< verify csv read >>
